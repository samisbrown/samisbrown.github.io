...
/////////////////
const cardImgContainerDiv = document.getElementById("cards_container");
cardImgs = [];
const previousBtn = document.getElementById("previous");
const rerollBtn = document.getElementById("reroll");
const cardAmountInp = document.getElementById("card_amount");
currentCardAmount = 0;
...
rerollBtn.addEventListener("click", () => {
	...
	for (i = 0; i < cardImgs.length; i++)
	{
		...
		console.log(card);
		cardImgs[i].src = GetCardImage(card);
	}
});
...
{
	for (i = currentCardAmount-1; i >= cardAmountInp.value; i--)
	{
		// Remove any cards
		cardImgs[i].remove();
		cardImgs.pop();
	}
	for (i = currentCardAmount; i < cardAmountInp.value; i++)
	{
		// Add any cards
		newImg = document.createElement("img");
		newImg.id = `card_img${i+1}`;
		newImg.classList.add("card");
		cardImgContainerDiv.appendChild(newImg);
		cardImgs.push(newImg);
	}
	currentCardAmount = parseInt(cardAmountInp.value);
};
cardAmountInp.addEventListener("change", updateNumberOfCards);
updateNumberOfCards();
