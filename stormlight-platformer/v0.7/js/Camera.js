class Camera {
	constructor(xy, bounds, centerAroundPlayer=false) { //bounds min x min y max x max y
		this.x = xy[0];
		this.y = xy[1];
		
		this.centerAroundPlayer = centerAroundPlayer;
		
		if (!this.centerAroundPlayer) {
			this.minx = Math.floor(bounds[0]);
			this.miny = Math.floor(bounds[1]);
			this.maxx = Math.floor(bounds[2]);
			this.maxy = Math.floor(bounds[3]);
		}
	...
	update() {
		...
		checkpoints.forEach(function(obj) {obj.dx = obj.x-this.x;}.bind(this));
		checkpoints.forEach(function(obj) {obj.dy = obj.y-this.y;}.bind(this));	
	}
